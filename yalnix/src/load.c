...
// ==>> of the new process.
    ...
    user_memory.text_high    = (unsigned int)((text_pg1 + li.t_npg) * PAGESIZE + VMEM_1_BASE);
    user_memory.data_low     = (unsigned int)(data_pg1 * PAGESIZE + VMEM_1_BASE);
    user_memory.data_high    = (unsigned int)((data_pg1 + data_npg) * PAGESIZE + VMEM_1_BASE);
    ...
    user_memory.stack_high   = (unsigned int)(cpp + stack_npg * PAGESIZE); 
    ...
    log_info("user_memory.brk_high => %p", user_memory.brk_high);
